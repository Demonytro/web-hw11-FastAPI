...
from src.schemas import ContactResponse, ContactIsActiveModel, ContactModel
...
async def get_contact_by_nick(body: ContactModel, db: Session = Depends(get_db)):
    contact = await repository_contacts.get_contact_by_nick(body.nick, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not found!")
    return contact
...
async def set_is_active_contact(body: ContactIsActiveModel, contact_id: int = Path(ge=1), db: Session = Depends(get_db)):
